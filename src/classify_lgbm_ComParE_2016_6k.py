...
def process_single_feature_set(feature_folder, train_ds, test_ds):
    ...
    # Conversão para DataFrame nomeando as colunas explicitamente para mitigar restrições de string no LGBM
    feature_names = [f"feat_{i}" for i in range(X_train_scaled.shape[1])]
    X_train_df = pd.DataFrame(X_train_scaled, columns=feature_names)
    X_test_df = pd.DataFrame(X_test_scaled, columns=feature_names)

    # Split interno estruturado para monitoramento do Early Stopping
    ...
    # Treinamento final associando os callbacks de Early Stopping (20 rodadas)
...
def run_pipeline():
    ...
    if test_ids is not None:
        ...
        # Nome do arquivo de saída configurado com o sufixo _LGBM
