...
# Pasta de feature analisada individualmente
...
def objective(trial, X_train, y_train):
    """
    Função objetivo para o Optuna com hiperparâmetros originais.
    ...
    # Corrigido para f1_macro e validação estratificada
...
def process_single_feature_set(feature_folder, train_ds, test_ds):
    """Mapeia rótulos, normaliza, tunna com Optuna e aplica threshold com desempate."""
    ...
    # --- EXTRAÇÃO DE PROBABILIDADES E CORREÇÃO DO THRESHOLD (MUDANÇA AQUI) ---
    probabilities = clf.predict_proba(X_test)[:, 1]

    # Adição de um ruído infinitesimal controlado para desempatar as previsões idênticas da árvore
    np.random.seed(42)
    jitter = np.random.uniform(-1e-7, 1e-7, size=probabilities.shape)
    stable_probs = probabilities + jitter

    # Encontra o ponto de corte perfeito usando os dados desempatados
    decision_threshold = np.median(stable_probs)
    y_pred = (stable_probs >= decision_threshold).astype(int)

    print(f" [OK] Feature {feature_folder} finalizada com Threshold de Desempate: {decision_threshold:.4f}")
...
def run_pipeline():
    ...
    if test_ids is not None:
        ...
        output_file = f'/home/dani/Documentos/PEP/single_feature_results_{FEATURE_TARGET}_DT.csv'
