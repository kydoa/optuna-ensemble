...
# Altere aqui para a feature alvo que deseja rodar no momento
FEATURE_TARGET = 'SpeechGraph_Full'
...
def objective(trial, X_train, y_train_encoded):
    ...
    param = {
        ...
        'random_state': 42,
        'verbosity': -1,
        'n_jobs': 1  # Mantém 1 thread por modelo interno durante a busca do Optuna
    }
    ...
    try:
        # Executa Cross-Validation com CV=4 otimizando a ACCURACY
...
def process_single_feature_set(feature_folder, train_ds, test_ds):
    ...
    # --- CODIFICAÇÃO MANDATÓRIA DE RÓTULOS (LightGBM exige inteiros/floats) ---
    label_map = {'HC': 0, 'AD': 1}
    ...
    # --- CONVERSÃO EM DATAFRAME PARA EVITAR CONFLITOS DE REGRAS DE STRING NO LGBM ---
    feature_names = [f"feat_{i}" for i in range(X_train_scaled.shape[1])]
    X_train_df = pd.DataFrame(X_train_scaled, columns=feature_names)
    X_test_df = pd.DataFrame(X_test_scaled, columns=feature_names)

    # Split interno para Early Stopping do LightGBM
    ...
    final_params.update({
        'random_state': 42,
        'verbosity': -1,
        'n_jobs': -1  # Treinamento final usa o máximo de núcleos da máquina
    })

    # Treinamento final com suporte a monitoramento de Early Stopping
    clf = LGBMClassifier(**final_params)

    # Callback manual para o Early Stopping estruturado na API do Scikit-Learn do LightGBM
    ...
    # Garantindo conversão limpa para listas nativas
...
def run_pipeline():
    ...
    if test_ids is not None:
        ...
        # Nome do arquivo final atualizado com o sufixo do LightGBM (_LGBM)
